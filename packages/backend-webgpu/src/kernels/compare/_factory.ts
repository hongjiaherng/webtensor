...
/**
 * Factory for element-wise broadcast comparisons. Structurally mirrors
 * `binary/_factory.ts` — same bindings (a, b, out, meta_a, meta_b) and
 * dispatch (one thread per output element). Output is hardcoded `array<u32>`
 * (bool on device). Inputs share one arithmetic dtype (f32 / i32), so the
 * pipeline cache keys on the input dtype rather than the output dtype.
 */
