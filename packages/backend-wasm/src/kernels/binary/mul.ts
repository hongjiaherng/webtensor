...
export const mulKernel: WASMKernel = (module, _node, inputs, outputs) => {
  ...
  try {
    ...
    const fnName = `mul_${dtypeSuffix(outputs[0].dtype)}_strided`;
    const fn = (module as unknown as Record<string, typeof module.mul_f32_strided>)[fnName];
    if (!fn) throw new Error(`mul: no WASM kernel for dtype ${outputs[0].dtype}`);
