...
export const addKernel: WASMKernel = (module, _node, inputs, outputs) => {
  ...
  try {
    ...
    const fnName = `add_${dtypeSuffix(outputs[0].dtype)}_strided`;
    const fn = (module as unknown as Record<string, typeof module.add_f32_strided>)[fnName];
    if (!fn) throw new Error(`add: no WASM kernel for dtype ${outputs[0].dtype}`);
