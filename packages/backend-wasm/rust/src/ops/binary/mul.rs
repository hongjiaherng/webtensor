...
/// Strided element-wise multiply. Meta layout: see `ops::BINARY_META_WORDS`.

#[wasm_bindgen]
pub fn mul_f32_strided(
    a_ptr: *const f32,
    b_ptr: *const f32,
    out_ptr: *mut f32,
    meta_ptr: *const u32,
...
pub fn mul_i32_strided(
    ...
    unsafe {
        ...
        for i in 0..total {
            let ai = strided_idx(shape, a_bcast, a_off, i as u32);
            let bi = strided_idx(shape, b_bcast, b_off, i as u32);
            out[i] = (*a_ptr.add(ai)).wrapping_mul(*b_ptr.add(bi));
        }
    }
}
