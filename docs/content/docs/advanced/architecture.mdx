...
- **High-level** — what 99% of users import. `tensor`, `zeros/ones/rand/randn`, `run()`, `compile(fn, spec)`, `grad(loss, param)`, `.equals()` / `.allclose()`. The training code pattern looks like JAX's `@jit` + `optax`.
