...
- `nn` / `optim` can depend on `core` (they compose tensor ops) but not on each other and not on any backend. They stay as optional layers on top — the core tensor library stands alone.
...
- **WASM** — Rust scalar loops compiled with `wasm-bindgen` (no `rayon`, no SIMD intrinsics). Memory uses `Vec` leak/reclaim across the JS/WASM boundary. Matmul is naive O(MNK), 2D-only, no tiling.
