...
- **Function name:** user-facing, in camelCase, using the common convention — `sum`, `mean`, `matmul`, `softmax`. Not `reduceSum` or `doMatrixMultiply`.
...
- **Parameter names:** `axis` (not `axes`), `keepdim` (not `keep_dims`), `dim` for single-axis ops.
