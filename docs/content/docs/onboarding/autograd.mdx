...
| --- | ------------------------------- | ------------------------------------------------------------------------------------------------------ |
| Add | `(grad, grad)`                  | ⚠️ **Missing unbroadcast** — wrong when shapes differ. See [Bugs and gaps](/docs/onboarding/bugs-and-gaps). |
...
The fix: a `sum`/`reduceSum` kernel that reduces along specified axes. The backward becomes `(unbroadcast(grad, a.shape), unbroadcast(grad, b.shape))`. Until reduce lands, **any model trained with broadcasting will produce wrong gradients silently**. See [Bugs and gaps](/docs/onboarding/bugs-and-gaps).
...
- **No optimizer.** The user is responsible for applying gradients to weights — there's no `SGD` / `Adam` yet (see [Roadmap](/docs/roadmap)).
