...
BACKENDS.forEach(({ name, create }) => {
  describe(`binary op broadcast gradients — ${name}`, () => {
    ...
    it('add: [3,1] + [1,4] → grads [3,1] and [1,4]', async () => {
      ...
      expect(await aGrad.equals(tensor([[4], [4], [4]]))).toBe(true);
      expect(await bGrad.equals(tensor([[3, 3, 3, 3]]))).toBe(true);
    });
    ...
    it('mul: [3,1] * [1,4] → grads are weighted sums', async () => {
      ...
      expect(await aGrad.equals(tensor([[100], [100], [100]]))).toBe(true);
      // dL/dB[0,j] = sum_i A[i,0] = 6
      expect(await bGrad.equals(tensor([[6, 6, 6, 6]]))).toBe(true);
    });
    ...
    it('scalar + vector: [1] + [5] → scalar grad aggregates', async () => {
      ...
      const aGrad = await run(a.grad!, { engine });
      const bGrad = await run(b.grad!, { engine });
      expect(await aGrad.equals(tensor([5]))).toBe(true); // a appears 5 times
      expect(await bGrad.equals(tensor([1, 1, 1, 1, 1]))).toBe(true);
    });
    ...
    it('rank-3 broadcast [2,3] + [4,1,3] → correct grad shapes', async () => {
      ...
      expect(await bGrad.equals(tensor([[[2, 2, 2]], [[2, 2, 2]], [[2, 2, 2]], [[2, 2, 2]]]))).toBe(
        true,
      );
    });
  });
