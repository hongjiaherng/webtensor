...
BACKENDS.forEach(({ name, create }) => {
  describe(`compile() — ${name} Backend`, () => {
    ...
    it('single output: traces forward and feeds tensors', async () => {
      ...
      expect(y.equals(tensor([[1.5, 0]]))).toBe(true);
    });
    ...
    it('feeds also accept raw TypedArray and nested arrays', async () => {
      ...
      expect(y2.equals(tensor([11, 22, 33]))).toBe(true);
    });
    ...
    it('is callable many times with different inputs', async () => {
      ...
      expect(y1.equals(tensor([1, 4, 9, 16]))).toBe(true);
      expect(y2.equals(tensor([100, 400, 900, 1600]))).toBe(true);
    });
    ...
    it('multi-output array preserves order', async () => {
      ...
      expect(s.equals(tensor([5, 7, 9]))).toBe(true);
      expect(prod.equals(tensor([4, 10, 18]))).toBe(true);
    });
    ...
    it('multi-output record returns keyed tensors', async () => {
      ...
      expect(total.equals(tensor([2, 3, 4, 5]))).toBe(true);
      expect(dot.equals(tensor([10]))).toBe(true);
    });
