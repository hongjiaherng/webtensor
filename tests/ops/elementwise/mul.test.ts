...
BACKENDS.forEach(({ name, create }) => {
  describe(`mul — ${name}`, () => {
    ...
    it('basic rank 1', async () => {
      ...
      expect(await y.equals(tensor([10, 18, 28]))).toBe(true);
    });

    it('scalar broadcast', async () => {
      const y = await run(mul(tensor([1, 2, 3, 4]), tensor([3])), { engine });
      expect(await y.equals(tensor([3, 6, 9, 12]))).toBe(true);
    });
    ...
    it('large tensor (1024 elements)', async () => {
      ...
      expect(await y.equals(tensor(Array.from({ length: 1024 }, () => 6.0)))).toBe(true);
    });
    ...
    it('identity (× 1)', async () => {
      ...
      expect(await y.allclose(tensor([5.5, 2.2, 1.1]))).toBe(true);
    });

    it('negative values', async () => {
      const y = await run(mul(tensor([-1, -2, 3]), tensor([4, -5, -6])), { engine });
      expect(await y.equals(tensor([-4, 10, -18]))).toBe(true);
    });
  });
});
