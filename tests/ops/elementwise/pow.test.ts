...
BACKENDS.forEach(({ name, create }) => {
  describe(`pow — ${name}`, () => {
    ...
    it('square (exponent=2)', async () => {
      ...
      expect(await y.allclose(tensor([1, 4, 9, 16]))).toBe(true);
    });

    it('cube (exponent=3)', async () => {
      const y = await run(pow(tensor([1, 2, 3]), 3), { engine });
      expect(await y.allclose(tensor([1, 8, 27]))).toBe(true);
    });

    it('exponent=0.5 (sqrt)', async () => {
      const y = await run(pow(tensor([4, 9, 16]), 0.5), { engine });
      expect(await y.allclose(tensor([2, 3, 4]), { atol: 1e-4 })).toBe(true);
    });
  });
});
