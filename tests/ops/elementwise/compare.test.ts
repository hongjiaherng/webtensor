...
BACKENDS.forEach(({ name, create }) => {
  describe(`comparison ops — ${name}`, () => {
    ...
    it('eq / ne on float32', async () => {
      ...
      expect(await e.equals(tensor([1, 0, 1, 0], { dtype: 'bool' }))).toBe(true);
      expect(await n.equals(tensor([0, 1, 0, 1], { dtype: 'bool' }))).toBe(true);
    });
    ...
    it('lt / le / gt / ge on float32', async () => {
      ...
      expect(await l.equals(tensor([1, 0, 0], { dtype: 'bool' }))).toBe(true);
      expect(await le_.equals(tensor([1, 1, 0], { dtype: 'bool' }))).toBe(true);
      expect(await g.equals(tensor([0, 0, 1], { dtype: 'bool' }))).toBe(true);
      expect(await ge_.equals(tensor([0, 1, 1], { dtype: 'bool' }))).toBe(true);
    });
    ...
    it('lt / gt on int32', async () => {
      ...
      const l = await run(lt(a, b), { engine });
      const g = await run(gt(a, b), { engine });
      expect(await l.equals(tensor([1, 0, 0, 0], { dtype: 'bool' }))).toBe(true);
      expect(await g.equals(tensor([0, 0, 1, 1], { dtype: 'bool' }))).toBe(true);
    });
    ...
    it('isclose: tight and loose tolerances', async () => {
      ...
      expect(await tight.equals(tensor([1, 0, 1, 1], { dtype: 'bool' }))).toBe(true);
      expect(await loose.equals(tensor([1, 1, 1, 1], { dtype: 'bool' }))).toBe(true);
    });
    ...
    it('isclose: NaN handling', async () => {
      ...
      expect(await def.equals(tensor([0, 0, 1], { dtype: 'bool' }))).toBe(true);
      expect(await eqNan.equals(tensor([1, 0, 1], { dtype: 'bool' }))).toBe(true);
    });
    ...
    it('isclose: ±inf matches itself', async () => {
      ...
      expect(await y.equals(tensor([1, 1, 0], { dtype: 'bool' }))).toBe(true);
    });
  });
});
