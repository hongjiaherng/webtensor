...
BACKENDS.forEach(({ name, create }) => {
  describe(`div — ${name}`, () => {
    ...
    it('large tensor (1024 elements)', async () => {
      ...
      expect(await y.equals(tensor(Array.from({ length: 1024 }, () => 3.0)))).toBe(true);
    });

    it('divide by one (identity)', async () => {
      const y = await run(div(tensor([7.5, 3.2, 1.1]), tensor([1, 1, 1])), { engine });
      expect(await y.allclose(tensor([7.5, 3.2, 1.1]))).toBe(true);
    });
  });
});
