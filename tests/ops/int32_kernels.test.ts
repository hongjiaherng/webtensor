...
// Phase 1: int32 kernels rollout. Kernels arrive backend-by-backend:
//   - CPU:    all ops (via dtype-polymorphic buf())
//   - WASM:   binary arithmetic (Phase 1c)
//   - WebGPU: binary arithmetic (Phase 1d) via SCALAR template substitution
//
// Each describe block filters to the backends that support it.

const ARITH_BACKENDS = BACKENDS; // binary arithmetic: all three backends
...
ARITH_BACKENDS.forEach(({ name, create }) => {
  describe(`int32 binary arithmetic — ${name}`, () => {
    ...
    it('int32 + int32 stays int32', async () => {
      ...
      expect(y.equals(tensor([11, 22, 33], { dtype: 'int32' }))).toBe(true);
    });
    ...
    if (name === 'CPU') {
      it('float32 + int32 promotes to float32 (CPU only, pending Phase 1e Cast)', async () => {
        ...
        expect(y.equals(tensor([2.5, 4.5]))).toBe(true);
      });
    }
    ...
    it('int32 sub / mul round-trip', async () => {
      ...
      expect(s.equals(tensor([9, 18, 27], { dtype: 'int32' }))).toBe(true);
      expect(m.equals(tensor([10, 40, 90], { dtype: 'int32' }))).toBe(true);
    });
    ...
    it('int32 div truncates toward zero', async () => {
      ...
      expect(y.equals(tensor([3, -3, 1], { dtype: 'int32' }))).toBe(true);
    });
...
CPU_ONLY.forEach(({ name, create }) => {
  describe(`int32 other ops (CPU-only for now) — ${name}`, () => {
    ...
    it('unary neg/abs preserve int32 dtype', async () => {
      ...
      expect(n.equals(tensor([3, 1, 0, -2, -5], { dtype: 'int32' }))).toBe(true);
      expect(b.equals(tensor([3, 1, 0, 2, 5], { dtype: 'int32' }))).toBe(true);
    });
    ...
    it('mean on int32 (currently stays int32)', async () => {
      ...
      expect(y.dtype).toBe('int32');
      expect(y.equals(tensor([4], { dtype: 'int32' }))).toBe(true);
    });
  });
});
