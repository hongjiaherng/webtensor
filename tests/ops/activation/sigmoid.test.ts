...
import { sigmoid } from '@webtensor/core';
...
BACKENDS.forEach(({ name, create }) => {
  describe(`sigmoid — ${name}`, () => {
    ...
    it('sigmoid(0) = 0.5', async () => {
      ...
      expect(await y.allclose(tensor([0.5]))).toBe(true);
    });

    it('sigmoid(10) ≈ 1', async () => {
      const y = await run(sigmoid(tensor([10])), { engine });
      expect(await y.allclose(tensor([1 / (1 + Math.exp(-10))]), { atol: 1e-4 })).toBe(true);
    });
    ...
    it('sigmoid of mixed values', async () => {
      ...
      expect(
        await y.allclose(tensor(vals.map((v) => 1 / (1 + Math.exp(-v)))), { atol: 1e-4 }),
      ).toBe(true);
    });
  });
});
