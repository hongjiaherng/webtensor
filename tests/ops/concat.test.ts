...
CONCAT_BACKENDS.forEach(({ name, create }) => {
  describe(`concat — ${name}`, () => {
    ...
    it('concat int32 tensors preserves dtype', async () => {
      ...
      expect(y.equals(tensor([1, 2, 3, 4, 5], { dtype: 'int32' }))).toBe(true);
    });
    ...
    it('backward splits grad into axis-slices', async () => {
      ...
      expect(aGrad.equals(tensor([1, 1, 1]))).toBe(true);
      expect(bGrad.equals(tensor([1, 1]))).toBe(true);
    });
    ...
    it('backward with weighted loss', async () => {
      ...
      const aGrad = await run(a.grad!, { engine });
      const bGrad = await run(b.grad!, { engine });
      expect(aGrad.equals(tensor([10, 20, 30]))).toBe(true);
      expect(bGrad.equals(tensor([40, 50]))).toBe(true);
    });
  });
});
